...
**Settings window front-ordering (v2.7.1):** the menu's "Settings…" is a `Button` calling `openSettingsWindow()` (NSApp `showSettingsWindow:` + `activate(ignoringOtherApps:)` + `orderFrontRegardless`), **not** `SettingsLink` — on an `LSUIElement` app `SettingsLink` opens the window without activating the app, so it surfaced *behind* other apps' windows. Same activate-on-open pattern as the Meeting Notes window.
