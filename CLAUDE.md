...
### Self-update (`AppUpdater.swift`, v2.7.1)

Settings → About shows an update control beside the version. `AppUpdater` checks GitHub `releases/latest`, semver-compares with the running `CFBundleShortVersionString`, and on a newer release runs `brew upgrade --cask templegit9/tap/meetingintro` **in-process** (via `/bin/zsh -lc`), then `open`s the upgraded bundle and quits to relaunch. **This works only because the app is NOT App-Sandboxed** — `MeetingIntro.entitlements` has the hardened-runtime/TCC entitlements (calendars, network.client, files.user-selected, device.audio-input) but no `com.apple.security.app-sandbox`, so spawning `brew` is permitted. If brew isn't found or the upgrade fails, it surfaces the manual `brew upgrade --cask meetingintro` command — never a dead end. Auto-checks once when About opens.
